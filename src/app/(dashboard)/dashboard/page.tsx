...
export default async function DashboardPage() {
  ...
  if (campaigns.length === 0) {
    return (
      <div className="mx-auto flex min-h-[70vh] max-w-lg flex-col items-center justify-center px-4 text-center">
        ...
        <h1 className="mt-2 text-3xl font-bold tracking-tight font-headline">Launch your first waitlist</h1>
        <p className="mt-3 text-[#64748B] font-small">Set up a public page, collect signups, and share one simple link. You can publish it the moment it is ready.</p>
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 p-4 pt-6 md:p-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-sm font-semibold text-[#FF2A54] font-small">Overview</p>
          <h1 className="text-3xl font-bold tracking-tight font-headline">Launch dashboard</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {stats.map(({ label, value, icon: Icon }) => (
          <div key={label} className="rounded-3xl border border-[#CBD5E1] bg-white p-5 shadow-sm">
            ...
            <p className="mt-3 text-3xl font-bold tracking-tight text-[#0F172A] font-headline">{value}</p>
          </div>
        ))}
      </div>

      <section className="rounded-3xl border border-[#CBD5E1] bg-white shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#CBD5E1] px-6 py-5">
        ...
        <div className="divide-y divide-[#CBD5E1]">
          {campaigns.slice(0, 5).map((campaign) => {
            const signups = campaign.signups?.[0]?.count ?? 0;
            return (
              <div key={campaign.id} className="flex flex-wrap items-center gap-4 px-6 py-4">
                <div className="min-w-40 flex-1"><Link href={`/dashboard/campaigns/${campaign.id}`} className="font-medium text-[#0F172A] hover:text-[#FF2A54] font-headline">{campaign.name}</Link><p className="mt-1 text-sm text-[#64748B] font-small">/c/{campaign.slug}</p></div>
                <div className="text-sm text-[#64748B] font-small"><span className="font-semibold text-[#0F172A]">{signups}</span> signups</div>
