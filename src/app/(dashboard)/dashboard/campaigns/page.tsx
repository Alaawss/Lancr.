    ...
    <div className="mx-auto max-w-7xl space-y-6 p-4 pt-6 md:p-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight font-headline">Your waitlists</h1>
      ...
      {campaigns.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-[300px] md:h-[400px] border rounded-2xl md:rounded-3xl border-dashed border-[#CBD5E1]">
          <div className="text-center space-y-4 px-4">
            <h3 className="text-base md:text-lg font-semibold font-headline">No campaigns yet</h3>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {campaigns.map((campaign) => {
            ...
            return (
              <div key={campaign.id} className="rounded-2xl md:rounded-3xl border border-[#CBD5E1] bg-white text-card-foreground shadow-sm">
                <div className="p-4 md:p-6 flex flex-col gap-3 md:gap-4">
                  <div className="flex justify-between items-start">
                    <div className="space-y-1">
                      <h3 className="font-semibold leading-none tracking-tight font-small text-sm md:text-base">{campaign.name}</h3>
                      <p className="text-xs md:text-sm text-[#64748B] font-small">{campaign.category}</p>
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 md:gap-4 border-y border-[#CBD5E1] py-3 md:py-4">
                    <div className="flex flex-col items-center">
                      <span className="text-xs text-[#64748B] flex items-center gap-1 font-small"><Eye className="h-3 w-3"/> Views</span>
                      <span className="font-bold font-headline text-sm md:text-base">{views}</span>
                    </div>
                    <div className="flex flex-col items-center">
                      <span className="text-xs text-[#64748B] flex items-center gap-1 font-small"><Users className="h-3 w-3"/> Signups</span>
                      <span className="font-bold font-headline text-sm md:text-base">{signups}</span>
                    </div>
                    <div className="flex flex-col items-center">
                      <span className="text-xs text-[#64748B] flex items-center gap-1 font-small"><BarChart className="h-3 w-3"/> Conv.</span>
                      <span className="font-bold font-headline text-sm md:text-base">{conversion}%</span>
                    </div>
                  </div>

                  <div className="flex gap-2">
                    <Link href={`/dashboard/campaigns/${campaign.id}`} className="flex-1"><Button variant="outline" className="w-full rounded-2xl text-sm">Manage</Button></Link>
